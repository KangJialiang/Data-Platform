...
def filter(gps_data):
  ...
  for i,line in enumerate(gps_data):
    if ((line[2]>31) & (line[1]<32) & (line[2]<122) & (line[2]>121)): 
      print(line)
...
def main():
  ...
  if args.odom_npys !="":
    ...
    odom_y = odom_poses[:,2]
    heading = args.heading * pi / 180
    rotate_odom_x = np.copy(odom_x) * cos(heading) - np.copy(odom_y) * sin(heading)
    rotate_odom_y = np.copy(odom_x) * sin(heading) + np.copy(odom_y) * cos(heading)
  ...
  if args.odom_npys != "":
    print(odom_x)
    plt.plot(rotate_odom_x, rotate_odom_y, label="monodepth2 odom", linewidth=1)
  if args.odom_scaled_npys != "":
    plt.plot(odom_scaled_x,odom_scaled_y, label="CNN scaled odom", linewidth=1)

  plt.axis('square')
  plt.legend()
  plt.savefig("mono_pose.png", bbox_inches='tight', dpi=300)
