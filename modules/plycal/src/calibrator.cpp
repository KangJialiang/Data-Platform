 ...
 *        3D point P_i_j locates at 3D edge i, its index is j. 3D edge i has
 * correspondense in image E_i(ax+by+c=0,a^2+b^2+c^2=1), project P_i_j into
 * image with initial T(q,p) and get 2D point P'_i_j(u,v). If T(q,p) is correct
 * or precise, P'_i_j should be on E_i. Thus, the error is the distance between
 * P'_i_j and E_i
 */
struct Edge2EdgeError {
   ...
   */
  Edge2EdgeError(const Eigen::Matrix3d& k, const Eigen::Matrix3Xd& ps,
                 const Eigen::Vector3d& cf)
      : K(k), pts(ps), coef(cf) {}
...
struct Point2PolygonError {
  ...
  Point2PolygonError(const Eigen::Matrix3d& k, const Eigen::Matrix3Xd& ps,
                     const Eigen::Matrix3Xd& vx)
...
uint32_t Calibrator::Add(const cv::Mat& img, const PCI& pc, cv::Mat& img_out,
...
bool Calibrator::RefineImage(const cv::Mat& img, cv::Mat& img_out,
...
bool Calibrator::RefinePointcloud(const PCI& pc, PCC& pcc,
    ...
    for (uint32_t i = 0; i < size_; i++) {
      ...
      ceres::CostFunction* cost =
          new ceres::AutoDiffCostFunction<Edge2EdgeError, 1, 4, 3>(
              new Edge2EdgeError(K_, ply.pc->edges[i].points,
                                 ply.img->edges[ply.ids[i]].coef));

      problem.AddResidualBlock(cost, loss_function_edge, q.coeffs().data(),
                               p.data());
    }
    // add inlier points
    ceres::CostFunction* cost =
        new ceres::AutoDiffCostFunction<Point2PolygonError, 1, 4, 3>(
            new Point2PolygonError(K_, ply.pc->inliers, ply.img->vertexs));
    problem.AddResidualBlock(cost, NULL, q.coeffs().data(), p.data());
    //        problem.AddResidualBlock(cost, loss_function_inlier,
...
void Calibrator::TrackLines(const Polygon& ply_prev, Polygon& ply) const {
  ...
  //            {
  //                offsets[i].second += prev.coef.cross(T*cur.points.col(k) -
  //                prev.p0).norm();
  //            }
  //        }
  //    }

  //    std::sort(offsets.begin(), offsets.end(), [](const PairUD& a, const
  //    PairUD& b)
  //    {
...
void Calibrator::MatchLines(Polygon& ply) const {
  ...
  std::sort(ids.begin(), ids.end(), [&ply](uint32_t a, uint32_t b) {
    return ply.pc->edges[a].points.cols() > ply.pc->edges[b].points.cols();
  });
  ...
  for (uint32_t i = 0; i < size_; i++) {
    ...
    offsets[i].second = 0;
    // 3d edge_0 <----> 2d edge i
    uint32_t id1 = (static_cast<int32_t>(i) + gap) < 0 ? i + gap + size_
                                                       : (i + gap) % size_;
  ...
  std::sort(
      ...
      [](const PairUD& a, const PairUD& b) { return a.second < b.second; });

  // ids[0] 3d edge <----> offsets[0].first 2d edge
  uint32_t id_img = offsets[0].first > ids[0]
                        ? offsets[0].first - ids[0]
...
void Calibrator::SavePolygonData(const std::string& dir) {
  ...
  for (const auto& ply : polygons_) {
    ...
    if (!pc_inlier.good() || !pc_edge.good() || !img_coef.good() ||
        !img_vertex.good()) {
      break;
    }
...
void Calibrator::Project(pcl::PointCloud<pcl::PointXYZRGB>& pc, cv::Mat& img,
  ...
  //    double depth_gap = std::sqrt(pt_max(0)*pt_max(0) + pt_max(1)+pt_max(1))
  //    - depth_gap + 0.01; if(depth_gap > 5)
  //    {
  //        depth_gap = 5.0;
  //    }
  ...
  for (auto& p : pc.points) {
    Eigen::Vector3d pt =
        K_ * (tf * Eigen::Vector4d(p.x, p.y, p.z, 1)).topRows(3);
    if (pt(2) < 0.5) {
      continue;
    }
