...
ImagePolygon::ImagePolygon(const nlohmann::json& js, uint8_t size)
  ...
  merge_endpoint_distance_threshold_ =
      js["merge"]["endpoint_distance_threshold"];
...
ImagePolygon::Polygon2D::Ptr ImagePolygon::InitPolygon(
  ...
  for (uint32_t i = 0; i < size_; i++) {
    verts[i].first = i;
    verts[i].second =
        std::atan2(pts(1, i) - center(1), pts(0, i) - center(0)) + MATH_PI;
  }
  std::sort(verts.begin(), verts.end(),
            [](PairUD& a, PairUD& b) { return a.second < b.second; });

  Polygon2D::Ptr ply = std::make_shared<Polygon2D>(size_);
  ply->center = center;

  auto EistimateCoef = [](const Eigen::Vector3d& p0, const Eigen::Vector3d& p1,
                          Eigen::Vector3d& coef) {
  ...
  for (uint32_t i = 0; i < size_; i++) {
    ...
    EistimateCoef(pts.col(verts[i].first), pts.col(verts[next].first),
                  ply->edges[i].coef);
    ply->edges[i].length =
        (pts.col(verts[i].first) - pts.col(verts[next].first)).norm();
    double dir = std::atan2(-ply->edges[i].coef(0), ply->edges[i].coef(1)) /
                 MATH_PI * 180;
    ply->edges[i].dir = (dir < 0) ? dir + 180 : dir;
    // ExtractLineFeature(img, pts.col(verts[i].first),
    // pts.col(verts[next].first), center, polygon_->edges[i].feature);
  }
...
ImagePolygon::Polygon2D::ConstPtr ImagePolygon::Init(
    const cv::Mat& img, cv::Mat& img_out, const Eigen::Matrix3Xd& pts) {
  Polygon2D::Ptr ply = InitPolygon(pts);
  //    SaveMarkedImage("init.png", img, ply);
  return ExtractPolygon(img, img_out, ply);
}

ImagePolygon::Polygon2D::ConstPtr ImagePolygon::Add(const cv::Mat& img,
                                                    cv::Mat& img_out) {
  if (polygon_ == nullptr) {
...
ImagePolygon::Polygon2D::ConstPtr ImagePolygon::ExtractPolygon(
    const cv::Mat& img, cv::Mat& img_out, Polygon2D::ConstPtr prev) {
...
bool ImagePolygon::ExtractLines(const cv::Mat& img, std::list<Line2D>& ls) {
  ...
  for (uint16_t i = 0; i < detector_->lineEquations_.size(); i++) {
    ...
    last.p0 << detector_->lineEndpoints_[i][0], detector_->lineEndpoints_[i][1],
        1;
    last.p1 << detector_->lineEndpoints_[i][2], detector_->lineEndpoints_[i][3],
        1;
    last.coef << detector_->lineEquations_[i][0],
        detector_->lineEquations_[i][1], detector_->lineEquations_[i][2];
    last.coef = last.coef / (last.coef.head(2).norm());

    last.points.resize(2,
                       detector_->lines_.sId[i + 1] - detector_->lines_.sId[i]);

    uint16_t col_index = 0;
    for (uint16_t j = detector_->lines_.sId[i];
         j < detector_->lines_.sId[i + 1]; j++) {
 ...
 */
bool ImagePolygon::FilterLines(std::list<Line2D>& ls,
...
bool ImagePolygon::MergeLines(std::list<Line2D>& ls) {
  ...
  while (ln != ls.end()) {
    ...
    while (ln_nx != ls.end()) {
      ...
      if (angle < merge_angle_threshold_ && d < merge_distance_threshold_ &&
...
bool ImagePolygon::FilterLinesByRectangule(std::list<Line2D>& ls,
                                           Polygon2D::ConstPtr prev) {
  assert(ls.size() >= 4);
  if (ls.size() < size_) {
  ...
  for (auto l = ls.cbegin(); l != ls.cend(); l++) {
    ...
    for (auto next = std::next(l); next != ls.cend(); next++) {
      ...
      if (err_angle < 10) {
        ...
        double angle_width =
            LineAngleError(angle_width_, LineAngleAverage(l->dir, next->dir));
        double angle_length =
            LineAngleError(angle_length_, LineAngleAverage(l->dir, next->dir));
  ...
  auto RectangleEdgesSort = [](const RectangleEdges& a,
                               const RectangleEdges& b) {
...
bool ImagePolygon::SortLines(std::list<Line2D>& ls, Polygon2D::ConstPtr prev,
                             Polygon2D::Ptr& ply) {
  if (ls.size() != size_) {
  ...
  ls.sort([](Line2D& a, Line2D& b) { return a.dir < b.dir; });
...
void ImagePolygon::UpdateParameters() { UpdateParameters(polygon_); }
...
void ImagePolygon::UpdateParameters(Polygon2D::Ptr ply) {
  ...
  filter_point_center_max_ =
      center2point.maxCoeff() * (1 + filter_point_center_factor_);
  filter_line_center_min_ =
      center2line.minCoeff() * (1 - filter_point_center_factor_);
...
void ImagePolygon::MarkImage(cv::Mat& img, const std::list<Line2D>& ls) {
  ...
  for (std::size_t i = 0; i < ls.size(); i++) {
    ...
    cv::putText(img, std::to_string(i), (p0 + p1) / 2, cv::FONT_HERSHEY_SIMPLEX,
                2, color, 2);
    lines_it++;
  }
