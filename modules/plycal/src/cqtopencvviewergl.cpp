...
CQtOpenCVViewerGl::CQtOpenCVViewerGl(QWidget* parent)
    : QOpenGLWidget(parent), is_pick_points_(false) {
  points_.reserve(4);
  ...
  if (!mRenderQtImg.isNull()) {
    ...
    glPushMatrix();
    {
      if (mResizedImg.width() <= 0) {
        if (mRenderWidth == mRenderQtImg.width() &&
            mRenderHeight == mRenderQtImg.height())
      ...
      glDrawPixels(mResizedImg.width(), mResizedImg.height(), GL_RGBA,
                   GL_UNSIGNED_BYTE, mResizedImg.bits());
    }
    glPopMatrix();
...
bool CQtOpenCVViewerGl::showImage(const cv::Mat& image, bool isInside) {
  ...
  mRenderQtImg =
      QImage((const unsigned char*)(mOrigImage.data), mOrigImage.cols,
             mOrigImage.rows, mOrigImage.step1(), QImage::Format_RGB32);
