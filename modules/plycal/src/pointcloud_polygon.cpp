...
bool PointcloudPolygon::ExtractRings(const PCI& pc, const Indices& indices,
  ...
  for (auto it : indices) {
    ...
    double pitch_d =
        std::atan(p.z / std::sqrt(p.y * p.y + p.x * p.x)) / MATH_PI * 180;
  ...
  for (auto& r : pts) {
    ...
    std::sort(ring.begin(), ring.end(),
              [](const Point& a, const Point& b) { return a.yaw < b.yaw; });
...
bool PointcloudPolygon::ExtractEdges(const PCI& pc, Indices& indices,
  ...
  if (ids.size() == indices.size()) {
    ...
    std::set_difference(indices.begin(), indices.end(), ids.begin(), ids.end(),
...
void PointcloudPolygon::FitLine(const PCI& pc, const Indices& indices,
                                Indices& inlier, Eigen::Vector3d& coef,
                                Eigen::Vector3d& pt) {
  PCI::Ptr pc_ptr(new PCI);
...
bool PointcloudPolygon::SaveMarkedPointcloud(
    const std::string& fn, const pcl::PointCloud<pcl::PointXYZI>& pc,
    Polygon3D::ConstPtr ply) const {
  pcl::PointCloud<pcl::PointXYZRGB> pcc;
  pcc.points.reserve(ply->inliers.cols() +
                     ply->edges[0].points.cols() * ply->edges.size());
